...
using personal_project.Models;//nhin thay thu muc Models
using X.PagedList;//phan trang
using personal_project.Areas.Admin.Attributes;//de nhin thay file CheckLogin.cs
...
namespace personal_project.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        {
            // lấy record tương ứng với id truyền vào để xoá dữ liệu
            ItemProduct record = db.Products.Where(item => item.Id == id).FirstOrDefault();
            if (record != null)
            {
